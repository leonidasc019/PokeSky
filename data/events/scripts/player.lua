...
function Player:onLook(thing, position, distance)
    ...
    if not thing:isItem() and isSummon(thing) then
        ...
        if master:isPlayer() then
            ...
            if item then
                ...
                local pokeEv = item:getSpecialAttribute("pokeEv") or 0
                local pokeGender = item:getSpecialAttribute("pokeGender")
                local pokeNature = item:getSpecialAttribute("pokeNature")
                local pokeBoost = item:getSpecialAttribute("pokeBoost") or 0

                -- Pegando atributos corretos da Pokébola
                local maxHealth = item:getSpecialAttribute("pokeMaxHealth") or 0
                local attack = item:getSpecialAttribute("pokeAttack") or 0
                local defense = item:getSpecialAttribute("pokeDefense") or 0
                local spAttack = item:getSpecialAttribute("pokeSpecialAttack") or 0
                local spDefense = item:getSpecialAttribute("pokeSpecialDefense") or 0
                local speed = item:getSpecialAttribute("pokeSpeed") or 0
                local ivString = item:getSpecialAttribute("pokeIV") -- Corrigido nome correto do atributo

                -- Se existir IVs, calcular a perfeição
                if ivString then
                    local ivs = {}
                    ...
                    if #ivs == 6 then
                        ...
                        -- Cálculo da perfeição dos IVs
                        local totalIV = ivHP + ivAtk + ivDef + ivSpAtk + ivSpDef + ivSpeed
                        local ivPerfection = (totalIV / 186) * 100

                        -- Formatação da descrição
                        if pokeName and pokeLevel then
                            description = string.format("%s\nIt belongs to %s. Level: %s. Boost: +%s.", description, master:getName(), pokeLevel, pokeBoost)
                            description = string.format(
                                "%s\nHealth: %s.\nAttack: %s.\nDefense: %s.\nSpecial Attack: %s.\nSpecial Defense: %s.\nSpeed: %s.\nEV-Points: %s.\nGender: %s.\nNature: %s.",
                                description, maxHealth, attack, defense, spAttack, spDefense, speed, pokeEv, pokeGender, pokeNature
                            )

                            -- Exibir IVs
                            description = string.format(
                                "%s\nIVs: HP: %d, Atk: %d, Def: %d, Sp.Atk: %d, Sp.Def: %d, Speed: %d.",
                                description, ivHP, ivAtk, ivDef, ivSpAtk, ivSpDef, ivSpeed
                            )

                            -- Exibir % de perfeição dos IVs
                            description = string.format("%s\nIV Perfection: %.2f%%", description, ivPerfection)
                        end
                    else
                        print("WARNING: IV string inválida, número incorreto de IVs!")
                    end
                end
            end
        end
    ...
    if thing:isItem() and thing:isPokeball() then
        ...
        local healthStr = ownerName and "It belongs to " .. ownerName .. "." or ""
    ...
    -- Exibir cargo se for um Tutor
    if thing:isPlayer() then
        if thing:getAccountType() == ACCOUNT_TYPE_TUTOR then
            local tutorStr = thing:getSex() == PLAYERSEX_MALE and "He is a Tutor." or "She is a Tutor."
            description = string.format("%s %s", description, tutorStr)
        end
    end

    -- Exibir informações adicionais para GMs
...
function Player:onTurn(direction)
	if hasSummons(self) then
		...
		summon:setIdle(6000)
		self:say(msg, TALKTYPE_MONSTER_SAY)
	end
	return true
end
...
--Itigar removido o up dos pokemons para um level fixo
function Player:onGainExperience(source, exp, rawExp)
    local multiplier = 400.0 -- O monstro ganha metade da experiência do jogador
	-- pokemon xp aqui experiencia pokemon
...
--		local item = self:getSlotItem(CONST_SLOT_AMMO)
		...
		if item and item:isPokeball() then
			...
			if newExp > getNeededExp(nextLevel)  then				
				...
				local pokeEv = item:getSpecialAttribute("pokeEv") 
				pokeEv = pokeEv + 1

				item:setSpecialAttribute("pokeLove", love)
				item:setSpecialAttribute("pokeEv", pokeEv)
