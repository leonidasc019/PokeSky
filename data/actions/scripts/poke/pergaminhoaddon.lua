function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target ~= player:getSummon() then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voce so pode usar o addon em um pokemon seu.")
		return false
	end
	...
	if target == player:getSummon() then
		...
		if string.find(player:getUsingBall():getSpecialAttribute("addons") or "", addonId) then
