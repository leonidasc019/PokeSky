...
-- Criando a tabela reversa
local reverseKits = {}
for box, furniture in pairs(constructionKits) do
    reverseKits[furniture] = box
end

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    -- Verifica se é uma caixa para virar móvel
    ...
    -- Verifica se é um móvel para virar caixa
    local box = reverseKits[item.itemid]
    if box then
        if not Tile(fromPosition):getHouse() then
            player:sendTextMessage(MESSAGE_STATUS_SMALL, "You can only wrap this item inside a house.")
        else
            item:transform(box)
