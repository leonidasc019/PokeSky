function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if not hasSummons(player) then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Seu pokemon precisa estar para fora para usar o addon.")
		return false
	end
	...
	if target ~= player:getSummon() then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voce so pode usar addon em um pokemon seu.")
		return false
	end
	...
	if target == player:getSummon() then
		...
		print(player:getUsingBall():getSpecialAttribute("addons"))
		if string.find(player:getUsingBall():getSpecialAttribute("addons"), addonId) then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Você ja possui esse addon!")
			return true
		end
		if addons ~= 0 then
