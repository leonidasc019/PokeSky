local storage = quests.marshBadge.prizes[1].uid
local pokemons = 
{
	[1] = 
		{
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		...
		if player then
			if player:getStorageValue(storage) <= 0 then
				selfSay('Eu nao gosto de lutar, mas se voce quiser, te mostrarei meus poderes!', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('Estou chocada, ' .. player:getName() .. "! Mas uma derrota e uma derrota. Eu admito que nao trabalhei duro o suficiente para vencer! Voce merece a Insignia da Lama", cid)
