local storage = quests.rainbowBadge.prizes[1].uid
local pokemons = 
{
	[1] = 
		{
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		...
		if player then
			if player:getStorageValue(storage) <= 0 then
				selfSay('Muito bem! Mas eu nao perderei!', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('Oh! Eu admito a derrota. Você e incrivelmente forte, ' .. player:getName() .. ". Eu devo lhe conferir a insignia do Arco-Iris.", cid)
