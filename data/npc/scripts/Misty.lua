local storage = quests.cascadeBadge.prizes[1].uid
local pokemons = 
{
	[1] = 
		{
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('Uau! Você e demais, ' .. player:getName() .. "! Tudo bem! Voce pode leva a Insignia da Cascata para mostrar que me derrotou!", cid)
