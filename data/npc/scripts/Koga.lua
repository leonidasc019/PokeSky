local storage = quests.soulBadge.prizes[1].uid
local pokemons = 
{
	[1] = 
		{
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		...
		if player then
			if player:getStorageValue(storage) <= 0 then
				selfSay('Muito bem, vou lhe mostrar o verdadeiro terror como um mestre ninja! Voce deve sentir o desespero das tecnicas de Veneno!', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('Humph! Voce provou seu valor, ' .. player:getName() .. "! Aqui! pegue a Insignia da Alma!", cid)
