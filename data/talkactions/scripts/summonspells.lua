...
function onSay(player, words, param)
    ...
    local storageCooldown = 12559

    local lastCommandTime = player:getStorageValue(storageCooldown)

    if lastCommandTime == -1 then
        lastCommandTime = 0
    end

    if currentTime - lastCommandTime < cooldownTime then
        -- player:sendCancelMessage("You must wait before using this command again.")
        return false
    end

    player:setStorageValue(storageCooldown, currentTime)
